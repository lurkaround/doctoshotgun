...
class Doctolib(LoginBrowser):
    ...
    def do_login(self):
        ...
        if self.page.redirect() == "/sessions/two-factor":
            ...
            try:
                self.challenge.go(
                    json={"auth_code": code, "two_factor_auth_method": "email"},
                    method="POST",
                )
            except HTTPNotFound:
    ...
    def try_to_book_place(
        ...
        elif isinstance(slot, str) and (
            vac_name == "janssen" or only_second or only_third
        ):
        ...
        if (
            vac_name != "janssen" and not only_second and not only_third
        ):  # janssen has only one shot
            self.second_shot_availabilities.go(
                params={
                    "start_date": slot_date_second.split("T")[0],
                    "visit_motive_ids": motive_id,
                    "agenda_ids": "-".join(agenda_ids),
            ...
            if self.page.is_error():
                log(
                    "  └╴ Appointment not available anymore :( %s",
                    self.page.get_error(),
                )
                return False
...
class Application:
    ...
    def main(self):
        ...
        doctolib_map = {"fr": DoctolibFR, "de": DoctolibDE}
        ...
        parser.add_argument(
            "--janssen", "-j", action="store_true", help="select only Janssen vaccine"
        )
        parser.add_argument(
            "--astrazeneca",
            "-a",
        ...
        parser.add_argument(
            "--only-second", "-2", action="store_true", help="select only second dose"
        )
        parser.add_argument(
            "--only-third", "-3", action="store_true", help="select only third dose"
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "--dry-run", action="store_true", help="do not really book the slot"
        )
        parser.add_argument(
        ...
        log(
            "Starting to look for vaccine slots for %s %s between %s and %s...",
        ...
        while True:
            try:
                ...
                print(
                    ...
                    % (
                        colored(
                            "Connection error. Check your internet connection. Retrying ...",
                            "red",
                        )
                    )
                )
                ...
                template = (
                    "An unexpected exception of type {0} occurred. Arguments:\n{1!r}"
                )
